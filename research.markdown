...
<details>
  <summary style="display: flex; align-items: center;">
    ...
    <div style="background-color: #e4e4f7; margin-top: 10px; padding: 4px; border-radius: 8px; width:96%;">
        ...
        <p>By using the Simulink model, we analyzed optimal damping forces that maximize energy output patterns for distinct sinusoidal acceleration inputs, each characterized by a different combination of signal amplitude and frequency. By observing that an optimal value for the damping force can exist based on input characteristics, we studied the potential harvested power from an extensive database of human arm and leg movement acceleration traces. Particularly, we devised an optimization procedure to identify the optimal damping force for various time-intervals of the acceleration traces, paving the way to study near-real-time tuning of the damping force.</p>
