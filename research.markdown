...
<details>
  <summary style="display: flex; align-items: center;">
    ...
    <div style="background-color: #e4e4f7; margin-top: 10px; padding: 4px; border-radius: 8px; width:96%;">
        ...
        <p>where the mass, spring, energy-extracting damping force, and internal displacement are represented by <em>m</em>, <em>k</em>, <em>f</em>, and <em>z</em>, respectively. To best suit highly-variable, infrequent motion from sensors and integrated energy harvesters attached to arms and legs, the energy-extracting damping mechanism comes from a form of electrostatic transduction done with a Coulomb-force parametric generator (CFPG) architecture. The damping foce in electrostatic transduction is set by the electric field bewteen the two capacitor plates on each side of the mass <em>m</em>, and this force can be optimized by controlling the input voltage source to the capacitor. With the CFPG architecture, the damping force <em>f</em> is constant and the spring constant <em>k</em> is zero. </p> 
        <p>Typically, the mass is attached to a moving capacitor plate that moves with respect to a fixed capacitor plate. When the gap between the two plates is at the smallest possible gap, the plates are pre-charged with low voltage from the input source, resulting in high capacitance (see '1-2' in the diagram below, from <a href="doi.org/10.1177/002029400804100404">doi.org/10.1177/002029400804100404</a>). As the external human kinetic motion's force exceeds the electrostatic damping force, the moving plate (and mass) breaks away and increases the gap between the capacitor plates. While this occurs, the voltage source is disconnected, keeping the charge on the plate constant while increasing voltage and decreasing capacitance (2-3). At the maximum possible gap, the voltage is at its highest and the capacitor is discharged to power-processing circuity (3-1). </p>
        ...
        <p>The electrostatic damping force must be less than the human kinetic motion's force for the mass to break away from a capacitor plate, and so optimal energy can be captured if the mass moves during the kinetic motion's peak force. To study the optimality of the damping force, we develoepd a Simulink model that capture the mass-spring-damper dynamics of the CFPG architecture as well as its resulting generated instantaneous power.</p>
        ...
        <p>By using the Simulink model, we analyzed optimal damping forces that maximize energy output patterns for distinct sinusoidal acceleration inputs, each characterized by a different combination of signal amplitude and frequency. By observing that an optimal value for the damping force exists based on the input's characteristics, we studied the potential harvested power from an extensive database of human arm and leg movement acceleration traces. Particularly, we devised an optimization procedure to identify the optimal damping force for various intervals of the acceleration traces, paving way to study possible near-real-time tuning of the damping force.</p>
        <center><img src="/assets/images/cfpg_sine_outputs.png.png" alt="Optimal value of electrostatic force and maximized harvested power for artificially-generated sinusoidal acceleration inputs." style="width: 350px; margin-bottom: 13px;"/></center>
    </div>
    <div style="background-color: #e4e4f7; margin-top: 10px; padding: 4px; border-radius: 8px; width:96%;">
        <h4 style="text-align: center; font-weight: bold; text-decoration: underline;">Key References</h4>
